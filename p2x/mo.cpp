...
void mo(Station* stations, int* stationsCoor, const int numberOfStations, int source){
	...
	acsetForMO(stations, numberOfStations, 2, numberOfStations/4, marked, unMarked, r);
	
	// 2nd
	for (int i=numberOfStations/4 + 1; i<=(numberOfStations/4) * 2; i++) {
		unMarked[stationHeap->getMin()]=i;
	}
	acsetForMO(stations, numberOfStations, numberOfStations/4 + 1, (numberOfStations/4) * 2, marked, unMarked, r);
	
	// 3rd
	for (int i=(numberOfStations/4) * 2 + 1; i<=(numberOfStations/4) * 3; i++) {
		unMarked[stationHeap->getMin()]=i;
	}
	acsetForMO(stations, numberOfStations, (numberOfStations/4) * 2 + 1, (numberOfStations/4) * 3, marked, unMarked, r);
	
	// 4th
	for (int i=(numberOfStations/4) * 3 + 1; i<=numberOfStations; i++) {
		unMarked[stationHeap->getMin()]=i;
	}
	acsetForMO(stations, numberOfStations, (numberOfStations/4) * 3 + 1, numberOfStations, marked, unMarked, r);
